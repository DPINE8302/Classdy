...
import { SlidersHorizontal, Palette, Calendar, Database, Trash2, Edit, Plus, Bell, AlertTriangle } from 'lucide-react';
import { SubjectManager } from './shared/SubjectManager';
import { SECRET_LOGS } from '../lib/secret-data';
...
const DataSettings: React.FC<Omit<SettingsProps, 'isOpen'|'onClose'>> = (props) => {
    ...
    const handleRestorePersonalData = () => {
        if (window.confirm("This will restore the developer's personal attendance logs and overwrite any existing log data. Are you sure?")) {
            props.replaceAllLogs(SECRET_LOGS);
            props.onShowMessage("Developer's attendance logs have been restored.");
        }
    };
            ...
            <div>
              <div className="mb-6">
               ...
               <div className="pt-6 border-t border-zinc-200 dark:border-zinc-700">
                <h4 className="font-semibold text-zinc-800 dark:text-zinc-200 mb-2">Secret Zone</h4>
                <p className="text-sm text-zinc-600 dark:text-zinc-400 mb-4">Restore the original developer's demo data. This will overwrite your current logs.</p>
                <button 
                  onClick={handleRestorePersonalData} 
                  className="w-full flex items-center justify-center gap-2 p-2.5 rounded-lg bg-amber-500/10 text-amber-600 dark:text-amber-400 font-semibold hover:bg-amber-500/20"
                >
                  <AlertTriangle size={16}/> Restore Personal Data
                </button>
              </div>
            </div>
        </div>
