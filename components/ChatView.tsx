...
import type { Schedule, SubjectMeta, Settings as SettingsType, ChatResponse, InteractiveSchedulePayload, InteractiveTasksPayload, ClassSession, InteractiveTask } from '../types';
...
export const ChatView: React.FC<ChatViewProps> = ({ schedules, subjectMeta, settings, lang }) => {
    ...
    const TasksCard: React.FC<{ payload: InteractiveTasksPayload, subjectMeta: SubjectMeta }> = ({ payload, subjectMeta }) => {
        const groupedTasks = payload.tasks.reduce((acc, task) => {
            ...
            return acc;
        }, {} as Record<string, { day: string; subject: string; tasks: InteractiveTask[] }>);
    ...
    return (
        <div className="flex flex-col h-full w-full bg-white dark:bg-zinc-800/50 rounded-2xl shadow-soft overflow-hidden">
