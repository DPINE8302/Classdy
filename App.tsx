...
export default function App() {
  ...
  // Apply theme, detect language, and check for first-time setup
  useEffect(() => {
  ...
  const renderContent = () => (
    schedules.length === 0 ? (
        ...
        <div className={`animate-fade-in ${isDesktop && activeTab === 'chat' ? 'flex-1 flex flex-col' : 'h-full'}`}>
          ...
          <div role="tabpanel" id="panel-chat" hidden={activeTab !== 'chat'} className={isDesktop ? 'flex-1 min-h-0' : 'h-full'}>
              {isDesktop ? (
                <ChatView
                  schedules={schedules}
                  subjectMeta={subjectMeta}
      ...
      return (
          <div className="h-screen w-screen flex bg-zinc-100 dark:bg-zinc-900 transition-colors duration-300 font-sans text-zinc-900 dark:text-zinc-50 antialiased">
              ...
              <div className="flex-1 flex flex-col overflow-hidden">
                  ...
                  <main className={`flex-1 p-8 ${activeTab === 'chat' ? 'flex flex-col' : 'overflow-y-auto'}`}>
                      {renderContent()}
                  </main>
              </div>
              {renderModals()}
  ...
  return (
    <div className="min-h-screen bg-zinc-100 dark:bg-zinc-900 transition-colors duration-300 flex flex-col">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex-grow w-full mb-24">
          {renderContent()}
      </main>
